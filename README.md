YT 筆記館 — 部署與使用說明
一、這是什麼
看完 YouTube 好影片 → 按「分享」→ 選「YT 筆記館」→ 網址自動帶入 → 寫心得、標三個時間重點、掛二層標籤 → 存入（日期自動記錄）。之後可用日曆或標籤找回來，按時間重點一鍵跳到影片該段。
二、部署到 GitHub Pages（與 YTToText 相同做法）
到 GitHub 的 `hsuleon99.github.io` repo，建一個資料夾，例如 `ytmemo/`
把這 6 個檔案全部上傳進去：
`index.html`
`manifest.json`
`sw.js`
...
`icon-192.png`
`icon-512.png`
等 1–2 分鐘，網址即為：`https://hsuleon99.github.io/ytmemo/`
三、手機安裝（關鍵一步）
Android（Chrome）：
用 Chrome 開啟上面網址
右上「⋮」→「加到主畫面」→「安裝」
安裝後，YouTube App 裡按「分享」，清單中就會出現「YT 筆記館」——按下去網址自動帶入，直接寫筆記
iPhone（Safari）：
Safari 開啟網址 → 分享鍵 →「加入主畫面」
iOS 的分享選單不支援 PWA share target，改用：YouTube 按分享 →「拷貝連結」→ 開 YT 筆記館 → 按「📋 貼上」即可（一樣只按一個鍵）
四、功能對照
需求	位置
...
步驟 1：建立 Firebase 專案
用您的 Google 帳號開啟 console.firebase.google.com
按「建立專案」→ 專案名稱輸入 `yt-memo`（或任何名字）→ 繼續
問要不要 Google Analytics → 關閉（用不到）→ 建立專案，等它轉完
步驟 2：開啟 Firestore 資料庫
左側選單「建構 (Build)」→「Firestore Database」→「建立資料庫」
位置 (Location) 選 asia-east1（台灣彰化機房）
模式選「正式版模式 (production mode)」→ 建立
步驟 3：設定安全規則（只有您本人能讀寫）
Firestore 頁面上方點「規則 (Rules)」分頁
把內容整個換成下面這段，然後按「發布 (Publish)」：
```
rules_version = '2';
...
「Sign-in method」分頁 → 點「Google」→ 開關切到「啟用」
「專案支援電子郵件」選您自己的信箱 → 儲存
步驟 5：加入授權網域（很重要，漏了會登入失敗）
Authentication →「設定 (Settings)」分頁 →「授權網域 (Authorized domains)」
按「新增網域」，輸入：`hsuleon99.github.io` → 新增
步驟 6：取得設定值
回到專案首頁，點齒輪 ⚙ →「專案設定」
往下捲到「您的應用程式」，點 `</>`（網頁） 圖示
應用程式暱稱隨便填（如 `yt-memo-web`），不要勾 Firebase Hosting → 註冊應用程式
畫面會出現一段 `const firebaseConfig = { apiKey: "...", ... }` — 這六個值就是您要的
步驟 7：貼進 firebase-config.js
打開 `firebase-config.js`，把六個「請貼上您的…」換成剛剛的真實值（照抄，含引號）
存檔，上傳到 GitHub 的 `yt-memo` 資料夾覆蓋
步驟 8：各裝置登入
每台裝置打開 App →「更多」→ 最上面「雲端同步」→「🔐 用 Google 登入啟用同步」
選同一個 Google 帳號
首次登入會自動把該裝置的本機資料合併上雲端（兩邊都不會少）
看到「☁️ 同步中：您的email」就完成了
之後任何一台裝置新增、修改、刪除、改標籤，其他裝置即時跟著變。
常見狀況
狀況	解法
